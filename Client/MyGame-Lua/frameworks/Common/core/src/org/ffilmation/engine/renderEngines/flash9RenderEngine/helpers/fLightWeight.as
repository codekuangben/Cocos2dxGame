package org.ffilmation.engine.renderEngines.flash9RenderEngine.helpers
{
	...
	public class fLightWeight
	{
		...
		{
			var per:Number = this.weight * light.color.alphaMultiplier;
			var ret:ColorTransform = new ColorTransform(light.color.redMultiplier * per, light.color.greenMultiplier * per, light.color.blueMultiplier * per, 1, light.color.redOffset * per, light.color.greenOffset * per, light.color.blueOffset * per, 0);
			return ret;
		}
		...
		public function updateWeight():void
		{
			...
			{
				if (angle > 180)
					this.weight = (225 - angle) / 45;
				else if (angle < 90)
					this.weight = ((45 - angle) / 45);
				this.weight = 1;
			}
			
			if (light.size != Infinity)
				this.weight *= (1 - (dist / light.size));
		}
	}
}