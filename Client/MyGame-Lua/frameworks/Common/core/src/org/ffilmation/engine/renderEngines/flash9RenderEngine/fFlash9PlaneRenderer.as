package org.ffilmation.engine.renderEngines.flash9RenderEngine
{
	...
	public class fFlash9PlaneRenderer extends fFlash9ElementRenderer
	{
		...
		private var m_needEnvironmentC:Boolean;
		//private var diffuseContainer:Sprite;	// 漫反射贴图容器    
		...
		private var m_imgInit:Boolean;		// 真正的地图资源是否初始化,如果是缩略图初始化而地图资源没有初始化这个就是 false
			...
			//{
				...
				//{
					...
					//{
						...
						//cl2 = c.length;
						//for (k2 = 0; k2 < cl2; k2++)
							//projectedShape[k2] = fScene.translateCoords(0, fEngine.RENDER_FINETUNE_3 + c[k2].x * fEngine.RENDER_FINETUNE_1 + c[k2].y * fEngine.RENDER_FINETUNE_2, fEngine.RENDER_FINETUNE_3 + c[k2].y * fEngine.RENDER_FINETUNE_1 + c[k2].x * fEngine.RENDER_FINETUNE_2);
				...
				//{
					...
					//{
						...
						//cl2 = c.length;
						//for (k2 = 0; k2 < cl2; k2++)
							//projectedShape[k2] = fScene.translateCoords(-0.0 + c[k2].x * fEngine.RENDER_FINETUNE_1 + c[k2].y * fEngine.RENDER_FINETUNE_2, 0, -0.0 + c[k2].y * fEngine.RENDER_FINETUNE_1 + c[k2].x * fEngine.RENDER_FINETUNE_2);
						//this.clipPolygon.contours[this.clipPolygon.contours.length] = projectedShape;
					//}
				//}
			//}
		...
		public override function createAssets():void
		{
			...
			// KBEN: 这段代码在 init 函数中进行了，但是这里要加载资源   
			...
			//{
				...
				// KBEN: 
				//if (this.scene.sceneConfig.mapType == fEngineCValue.EngineISO)
				//{
			...
			//}
			// 先创建一个 diffuse 不用赋值图像资源，这个先注释掉    
			//this.diffuse = new Bitmap(null, "never", true);
			...
			// KBEN: bug 加载资源一定要放在 this.baseContainer = objectPool.getInstanceOf(Sprite) as Sprite; 这行代码后面，否则 undoCache 中 this.spriteToDraw.addChildAt(this.baseContainer, 0); 这行由于 baseContainer 为空宕机   
			//element.loadRes();
			...
			// KBEN:
			//this.diffuseContainer = objectPool.getInstanceOf(Sprite) as Sprite;
			...
			//this.baseContainer.addChild(this.diffuseContainer);
			...
			// 初始化绘制黑色 
			if (this.element.scene.m_sceneConfig.fogOpened)
			{
				var floor:fFloor = this.element as fFloor;
				this.m_fogSprite.graphics.beginFill(0x000000);
				this.m_fogSprite.graphics.drawRect(0, 0, floor.width, floor.depth);
		...
		private function createLightC():void
		{
			if (lightC)
			{
				return;
			}
			m_needEnvironmentC = true;
			...
			// KBEN: lightC 中各个子 Sprite 中的颜色相乘    
			...
			{
				var contours:Array = (this.element as fPlane).shapePolygon.contours;
		...
		public override function destroyAssets():void
		{
			...
			//{
				...
				//if (!element.holes[i].open && element.holes[i].block && this.behind)
					//this.behind.removeChild(element.holes[i].block);
			//}
		...
		{
			// Already cached
			//if (this.finalBitmap.parent || this.anyClosedHole)
			...
			//{
				...
				//{
					//if (this.lightShadowsPl[i])
						//this.lightShadowsPl[i].filters = [blur];
				//}
			//}
			...
			this.finalBitmapData = new BitmapData(this.element.bounds2d.width, this.element.bounds2d.height, false, 0);
			
			// Draw
			var oMatrix:Matrix = new Matrix();
			// KBEN: 斜 45 度已经是向上移动了 this.diffuse.y 因此要绘制的时候向下移动才行  
			if (this.scene.m_sceneConfig.mapType == fEngineCValue.EngineISO || 
				this.scene.m_sceneConfig.mapType == fEngineCValue.Engine25d)
			{
				oMatrix.translate(0, -int(this.diffuse.y));
			}
			...
			this.finalBitmap.y = int(this.diffuse.y);
			// KBEN: 将整个面板的内容显示到屏幕上    
		...
		private function redrawShadowsOnShowHide(e:Event = null):void
		{
			...
			//{
				...
				//if (light && !light.removed && this.element.distanceTo(light.x, light.y, light.z) < light.size)
					//light.render();
			//}
		}
		...
		{
			// 感觉灯光内部 sprite 颜色计算公式就是 alpha1 * color1 + aplha2 * color2 ，然后再和这个 aprite 的 parent 的颜色进行 multiply 运算  
			// KBEN: 首先绘制黑色背景，模拟全局光照    
			if (light.hexcolor == 0xffffff && light.intensity==100)
			{
				if (m_needEnvironmentC == false)
				{
					return;
				}
			}
			...
			// KBEN: 测试全局光照 light.intensity 改为 0 场景全是黑色 ，改为 1 才正常显示 diffuse 贴图颜色 
			// Environment
		...
		private function newMaterial(evt:fNewMaterialEvent):void
		{
			...
			//if (fFlash9FloorRenderer.floorProjectionCache.fl == p)
				//fFlash9FloorRenderer.floorProjectionCache.fl = null;
			//for (var i:String in fFlash9FloorRenderer.wallProjectionCache)
			//{
				//if (int(i.substring(i.indexOf("_") + 1)) == p.uniqueId)
					//fFlash9FloorRenderer.wallProjectionCache = null;
			//}
			...
			// Holes
			//while (this.behind.numChildren > 0)
				//this.behind.removeChild(this.behind.getChildAt(0));
			//this.processHoles(p);
			//if (this.canBeSmoothed)
				//for (var n:String in this.lightShadowsPl)
					//this.lightShadowsPl[n].blendMode = BlendMode.NORMAL;
			//else
				//for (n in this.lightShadowsPl)
					//this.lightShadowsPl[n].blendMode = BlendMode.ERASE;
		...
		public override function renderStart(light:fLight):void
		{
			...
			//if (!this.lightStatuses[light.uniqueId])
				//this.lightStatuses[light.uniqueId] = new fLightStatus(this.element as fPlane, light);
		...
		//{
			...
			//{
				...
				//}
				//
				//if (fEngine.bumpMapping && light.bump && this.bumpMap)
					//radius *= 0.85;
				//
				...
				//var minimumArea:vport = new vport();
				...
				//{
					//if (minimumArea.x_min < this.element.bounds2d.x)
						//minimumArea.x_min = this.element.bounds2d.x;
					//if (minimumArea.y_min < this.element.bounds2d.y)
						//minimumArea.y_min = this.element.bounds2d.y;
				//}
				//
				...
				//var cl:int = contours.length;
				//for (var k:int = 0; k < cl; k++)
					//polygonToDraw.contours[k] = polygonUtils.clipPolygon(contours[k], minimumArea);
				//var holes:Array = this.clipPolygon.holes;
				//var hl:int = holes.length;
				//for (k = 0; k < hl; k++)
					//polygonToDraw.holes[k] = polygonUtils.clipPolygon(holes[k], minimumArea);
				//
			//}
			//else
			//{
		...
		//{
			...
			//{
				//
				//if (!(other.customData.flash9Renderer as fFlash9ObjectRenderer).eraseShadows)
		...
		//{
			...
			//{
				...
				//{
					...
					// restore cache if the object doesn't change for a few seconds.
					//this.cacheTimer.stop();
					//if (this.container.cacheAsBitmap == true)
						//this.undoCache();
					//
					// Start cache timer
					//this.cacheTimer.start();
		...
		*/
		...
		// KBEN: 获取一个精灵进行坐标变换
		public function get spritePar():Sprite
		{
			return spriteToDraw;
		}
		...
		override public function init(res:SWFResource, act:uint, direction:uint):void
		{
			...
			// 初始化自己的数据    
			//var d:DisplayObject = element.material.getDiffuse(element, element.bounds2d.width, element.bounds2d.height, true);
			...
			if (d)
			{
				...
				//if(!this.diffuseData)
				//{
				//	diffuseData = new BitmapData(element.bounds2d.width, element.bounds2d.height, false, 0);
				//}
				//var oMatrix:Matrix = this.planeDeform.clone();
				// KBEN: 
				//if (this.scene.m_sceneConfig.mapType == fEngineCValue.EngineISO ||
				//	this.scene.m_sceneConfig.mapType == fEngineCValue.Engine25d)
				//{
				//	oMatrix.translate(0, -int(Math.round(element.bounds2d.y)));
				//}
				//diffuseData.draw(d, oMatrix);
				this.diffuseData = d; 	// 赋值资源
				if (!this.diffuse)
				{
					//this.diffuse = new Bitmap(diffuseData, "never", true);
					this.diffuse = new Bitmap(diffuseData, "auto", true);
					//this.diffuseContainer.addChild(this.diffuse);
				}
				else
				{
				...
				//var ba:ByteArray = encode.encode(nDiffuseData);
				//var file:FileReference = new FileReference();
				//file.save(ba, "E:/aaa.jpg");
			}
			else
			{
				//this.diffuse.bitmapData = new BitmapData(1, 1, true, 0);
		...
		/*
		// KBEN: 清理当前点的雾  ptx pty 是 floor 局部空间坐标   
		public function clearFog(ptx:int, pty:int, autoStart:Boolean = true):void
		{
			if (this.element.scene.m_sceneConfig.fogOpened)
			{
				this.m_fogSprite.graphics.beginFill(0xFFFFFF);
				this.m_fogSprite.graphics.drawCircle(ptx, pty, 100);
				this.m_fogSprite.graphics.endFill();
				
				// KBEN: 初始化完成后，缓存一下数据，数据改变重新缓存数据   
				this.undoCache(autoStart);
			}
		}
		*/
		...
		/*
		// test: 测试保存图片 
		public function savePic(idx:uint = 0):void
		{
			var element:fPlane = this.element as fPlane;
			var encode:JPGEncoder = new JPGEncoder(90);
			var file:FileReference = new FileReference();
			...
			{
				// KBEN: 打印测试     
				ba = encode.encode(this.diffuseData);
				file.save(ba, "aaa.jpg");
			}
			else
			{
				var bt:BitmapData = new BitmapData(this.origWidth, this.origHeight);
				var d:DisplayObject = element.material.getDiffuse(element, this.origWidth, this.origHeight, true)
				bt.draw(d);
				ba = encode.encode(bt);
				file.save(ba, "bbb.jpg");
			}
		}
		*/
		
		// 地形缩略图设置
		protected function initThumbnails():void
		{
			var eleplane:fPlane = this.element as fPlane;
			//if(!eleplane.scene.m_thumbnailsExtend)
			if(!eleplane.scene.m_thumbnails)
				return;
			
			// bug：有时候如果地形资源没有加载完成就进入这里，通常是由于摄像机的移动导致更新地形，从而初始化这个函数
			try
			{
				var errortst:uint = eleplane.scene.m_thumbnails.width;
			}
			catch (e:Error)
			{
				return;
			...
			// 判断地图偏移
			if(this.scene.m_scenePixelXOff <= eleplane.x && eleplane.x < this.scene.m_scenePixelWidth + this.scene.m_scenePixelXOff 
			   && this.scene.m_scenePixelYOff <= eleplane.y && eleplane.y < this.scene.m_scenePixelHeight + this.scene.m_scenePixelYOff)
			{
				this.diffuseData = new BitmapData(element.bounds2d.width, element.bounds2d.height, false, 0);
				var oMatrix:Matrix = this.planeDeform.clone();
				// KBEN: 
				if (this.scene.m_sceneConfig.mapType == fEngineCValue.EngineISO ||
					this.scene.m_sceneConfig.mapType == fEngineCValue.Engine25d)
				...
				//oMatrix.translate(-int(eleplane.x - this.scene.m_scenePixelXOff), -int(eleplane.y - this.scene.m_scenePixelYOff));
				oMatrix.scale(this.scene.m_scenePixelWidth/this.scene.m_thumbnails.width, this.scene.m_scenePixelHeight/this.scene.m_thumbnails.height);
				oMatrix.translate(-int(eleplane.x - this.scene.m_scenePixelXOff), -int(eleplane.y - this.scene.m_scenePixelYOff));
				//this.diffuseData.draw(eleplane.scene.m_thumbnailsExtend, oMatrix);
				this.diffuseData.draw(eleplane.scene.m_thumbnails, oMatrix);
				if (!this.diffuse)
				{
					//this.diffuse = new Bitmap(diffuseData, "never", true);
					this.diffuse = new Bitmap(diffuseData, "auto", true);
