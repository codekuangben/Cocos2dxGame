package org.ffilmation.engine.logicSolvers.collisionSolver.collisionModels
{
	...
	import org.ffilmation.engine.datatypes.fCilinder;
	import org.ffilmation.engine.datatypes.fPoint3d;
	import org.ffilmation.utils.lineCircleIntersectionResult;
	...
	public class fCilinderCollisionModel extends fCilinder implements fEngineCollisionModel
	{
		...
		public function testSegment(x1:Number, y1:Number, z1:Number, x2:Number, y2:Number, z2:Number):fPoint3d
		{
			...
			else
			{
				...
				{
					if (x1 == x2)
						inter1 = mathUtils.linesIntersect(-1, z1, -2, z2, 0, 0, 0, this._height);
					else
						inter1 = mathUtils.linesIntersect(x1, z1, x2, z2, x1, 0, x2, this._height);
				}
				else if (r.enter)
				{
					if (x1 == x2)
						var inter1:Point = mathUtils.linesIntersect(-1, z1, -2, z2, r.enter.x, 0, r.enter.x, this._height);
					else
						inter1 = mathUtils.linesIntersect(x1, z1, x2, z2, r.enter.x, 0, r.enter.x, this._height)
					if (inter1)
						return new fPoint3d(r.enter.x, r.enter.y, inter1.y);
				}
				else
				{
					if (x1 == x2)
						inter1 = mathUtils.linesIntersect(-1, z1, -2, z2, r.exit.x, 0, r.exit.x, this._height);
					else
						inter1 = mathUtils.linesIntersect(x1, z1, x2, z2, r.exit.x, 0, r.exit.x, this._height);
					if (inter1)
						return new fPoint3d(r.exit.x, r.exit.y, inter1.y);
				}
			}
			return null;
