	...
	public class fPlaneTesselation
	{
		...
		public static function tesselateFloor(f:fFloor, cubeSize:int):Array
		{
			...
			if (startCube != endCube)
			{
				...
				xml1.@id += "_upSplit";
				if (xml1.@height >= (f.scene.gridSize / 2))
				...
				if (xml2.@height >= (f.scene.gridSize / 2))
			...
			for (var i:int = 0; i < l; i++)
			{
				...
				if (startCube != endCube)
				{
					...
					xml1.@id += "_leftSplit";
					if (xml1.@width >= (f.scene.gridSize / 2))
					...
					if (xml2.@width >= (f.scene.gridSize / 2))
		...
		public static function tesselateWall(w:fWall, cubeSize:int):Array
		{
			...
			if (w.vertical)
			{
				...
				if (startCube != endCube)
				{
					...
					xml1.@id += "_upSplit";
					if (xml1.@size >= (w.scene.gridSize / 2))
					...
					if (xml2.@size >= (w.scene.gridSize / 2))
			...
			for (var i:int = 0; i < l; i++)
			{
				...
				if (startCube != endCube)
				{
					...
					xml1.@id += "_lowerSplit";
					if (xml1.@height >= (w.scene.levelSize / 2))
					...
					if (xml2.@height >= (w.scene.levelSize / 2))
