...
package org.ffilmation.engine.core.sceneLogic
{
	...
	public class fLightSceneLogic
	{
		...
		public static function processNewCellOmniLight(scene:fScene, light:fOmniLight, forceReset:Boolean = false):void
		{
			...
			for (var i2:int = 0; i2 < nEl; i2++)
				scene.renderEngine.lightOut(light.elementsV[i2].obj, light);
			...
			else
			{
				...
				if (!light.cell.lightAffectedElements || light.cell.lightRange < light.size)
					scene.getAffectedByLight(light.cell, light.size);
			...
			for (i2 = 0; i2 < nElements; i2++)
			{
				...
				ele = tempElements[i2];
				if (ele.distance < light.size)
					scene.renderEngine.lightIn(ele.obj, light);
			...
			for (i2 = 0; i2 < chLength; i2++)
			{
				...
				if (character.cell)
					character.cell.characterShadowCache[light.counter] = cache;
			}
		}
		...
		{
			//if (scene.prof)
			//	scene.prof.begin("Render light:" + light.id, true);
				
			if (scene.engine.m_context.m_profiler)
				scene.engine.m_context.m_profiler.enter("Render light:" + light.id);
			...
			for (var i2:Number = 0; i2 < nElements; i2++)
				scene.renderEngine.renderStart(tempElements[i2].obj, light);
			...
			for (i2 = 0; i2 < nElements; i2++)
			{
				...
				//if (scene.prof)
				//	scene.prof.begin("Element: " + el.id);
				if (scene.engine.m_context.m_profiler)
					scene.engine.m_context.m_profiler.enter("Element: " + el.id);
				...
				//if (scene.prof)
				//	scene.prof.end("Element: " + el.id);
				if (scene.engine.m_context.m_profiler)
					scene.engine.m_context.m_profiler.exit("Element: " + el.id);
			}
			...
			for (idChar = 0; idChar < len; idChar++)
			{
				...
				if (cache && cache.withinRange)
				{
					...
					//if (scene.prof)
					//	scene.prof.begin("Character: " + character.id);
					if (scene.engine.m_context.m_profiler)
						scene.engine.m_context.m_profiler.enter("Character: " + character.id);
					...
					for (i2 = 0; i2 < ell; i2++)
					{
						try
						{
							if (character._visible)
								scene.renderEngine.renderShadow(elements[i2], light, character);
						}
						catch (e:Error)
						{
					...
					//if (scene.prof)
					//	scene.prof.end("Character: " + character.id);
					if (scene.engine.m_context.m_profiler)
						scene.engine.m_context.m_profiler.exit("Character: " + character.id);
				}
			...
			// Step 5: End render
			for (i2 = 0; i2 < nElements; i2++)
				scene.renderEngine.renderFinish(tempElements[i2].obj, light);
			
			//if (scene.prof)
			//	scene.prof.end("Render light:" + light.id);
			if (scene.engine.m_context.m_profiler)
				scene.engine.m_context.m_profiler.exit("Render light:" + light.id);
		}
		*/
	}
}