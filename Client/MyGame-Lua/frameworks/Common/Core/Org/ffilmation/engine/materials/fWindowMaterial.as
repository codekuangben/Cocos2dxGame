package org.ffilmation.engine.materials
{
	...
	public class fWindowMaterial implements fEngineMaterial
	{
		...
		public function fWindowMaterial(definition:fMaterialDefinition):void
		{
			...
			try
			{
				this.geometryW = new Number(t.split("x")[0])
			}
			catch (e:Error)
			{
				this.geometryW = 1
			...
			try
			{
				this.geometryH = new Number(t.split("x")[1])
			}
			catch (e:Error)
			{
				this.geometryH = 1
		...
		private function calcWindows(width:Number, height:Number):void
		{
		...
		public function getDiffuse(element:fRenderableElement, width:Number, height:Number):BitmapData
		{
			...
			if (framesize > 0 && this.definition.xmlData.frame)
			{
				...
				if (element is fWall && (element as fWall).vertical)
					angle = 315;
				
				var fil:DropShadowFilter = new DropShadowFilter(3, angle, 0, 1, 5, 5, 1, BitmapFilterQuality.HIGH);
				temp2.filters = [fil];
			}
