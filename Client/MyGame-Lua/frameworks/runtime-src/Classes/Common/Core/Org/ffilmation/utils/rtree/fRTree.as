	...
	public class fRTree
	{
		...
		public function fRTree():void
		{
			...
			for (var i:int = 0; i < this.maxNodeEntries; i++)
				this.initialEntryStatus[i] = fRTree.ENTRY_STATUS_UNASSIGNED;
		...
		private function chooseNode(r:fCube, level:int):fRTreeNode
		{
			...
			while (true)
			{
				...
				for (var i:int = 1; i < n.entryCount; i++)
				{
					...
					if ((tempEnlargement < leastEnlargement) || ((tempEnlargement == leastEnlargement) && (tempRectangle.volume() < n.getEntry(index).volume())))
					{
						index = i;
		...
		private function adjustTree(n:fRTreeNode, nn:fRTreeNode):fRTreeNode
		{
			...
			while (n.level != this.treeHeight)
			{
				...
				{
					trace("Error: entry " + entry + " in node " + parent.nodeId + " should point to node " + n.nodeId + "; actually points to node " + parent.ids[entry]);
				}
