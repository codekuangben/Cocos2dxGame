	...
	public class fSceneGridSorter extends EventDispatcher
	{
		...
		public function createGrid():void
		{
			...
			for (var fi:int = 0; fi < fl; fi++)
			{
				...
				if (f.k == 0)
				{
					...
					{
						// KBEN: this.scene.grid[i] 就是 i 列所有的格子
						if (!this.scene.grid[i])
			...
			this.scene.grid = new Array;
			this.scene.allUsedCells = new Array;
			var fl:int = this.scene.gridWidth;
			for (var fi:int = 0; fi < fl; fi++)
			{
				// KBEN: this.scene.grid[fi] 就是 fi 列所有的格子
				this.scene.grid[fi] = new Array;
			}
		...
		public function start():void
		{
			...
			for (var i:int = 0; i < cwidth; i++)
			{
				...
				for (var j:int = 0; j < cdepth; j++)
				{
					...
					//  n.zIndex = ((((((cwidth - i + 1) + (j * cwidth + 2))) * cheight) + k)) / (cwidth * cdepth * cheight)
						// KBEN: 去掉三维，设置 cheight = 1, k = 0
						n.zIndex = ((cwidth - i + 1) + (j * cwidth + 2)) / (cwidth * cdepth);
					//	n.walls = []
						n.floors = [];
					//	this.sortCubes[i][j][k] = n
						this.sortCubes[i][j] = n;
			...
			//{
				...
				//if (w.vertical)
					//this.allVerticals[this.allVerticals.length] = w;
				//else
					//this.allHorizontals[this.allHorizontals.length] = w;
			//}
			...
			for (i = 0; i < this.scene.floors.length; i++)
			{
				...
				this.sortCubes[Math.floor((f.x + 2) / this.scene.sortCubeSize)][Math.floor((f.y + 2) / this.scene.sortCubeSize)].floors.push(f);
			}
		...
		private function zSortCube():void
		{
			...
			sl = buffer.length;
			for (i = 0; i < sl; i++)
				(buffer[i] as fPlane).setZ((i + 1) / (sl + 2));
		...
		private function zSortCubeComplete(event:TimerEvent):void
		{
			...
			if (this.cubeBeingProcessed < this.serializedSortCubes.length)
				this.zSortCube();
			else
				this.zSortComplete();
		}
		...
		private function zSortComplete():void
		{
			...
			for (var i:int = 0; i < sl; i++)
				(sortArray[i] as fPlane).setZ(i + 1);
			...
			for (var j:int = 0; j < ol; j++)
				(this.scene.objects[j] as fSceneObject).updateDepth();
			
			var cl:int = this.scene.characters.length;
			for (j = 0; j < cl; j++)
				(this.scene.characters[j] as fCharacter).updateDepth();
