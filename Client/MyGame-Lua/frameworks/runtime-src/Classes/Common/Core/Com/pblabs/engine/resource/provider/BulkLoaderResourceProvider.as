package com.pblabs.engine.resource.provider
{
    ...
    public class BulkLoaderResourceProvider extends ResourceProviderBase
    {
         ...
         * Constructor
         */
        public function BulkLoaderResourceProvider(name:String, numConnections:int = 12, registerAsProvider:Boolean=true, context:Context = null)
        {
            super(registerAsProvider, context);									 
		...
		public override function getResource(uri:String, type:Class, forceReload:Boolean = false, ver:String = "", context:LoaderContext=null):Resource
        {
			...
			if(ver && ver.length)
			{
				//loader.add(uri, { id : resourceIdentifier, type: type == MP3Resource ? "sound" : "binary", "preventCache" : 1, "urlparam" : ver } );
				loader.add(uri, { id : resourceIdentifier, type: type == SWFResource ? "swf" : type == MP3Resource ? "sound" : "binary", "preventCache" : 1, "urlparam" : ver,"context":context } );
			}
			else
			{
				//loader.add(uri, { id : resourceIdentifier, type: type == MP3Resource ? "sound" : "binary"  } );
				loader.add(uri, { id : resourceIdentifier, type: type == SWFResource ? "swf" : type == MP3Resource ? "sound" : "binary" ,"context":context } );
			}
			...
			// KBEN: 这几个事件好像没有移除事件 bug: 内存泄露,资源不能释放
			...
			loader.get(resourceIdentifier).addEventListener(BulkLoader.PROGRESS,resourceProgress);
			loader.get(resourceIdentifier).addEventListener(BulkLoader.OPEN,resourceStarted);
        ...
        private function resourceLoaded(event:Event):void
        {
            ...
            if (resources[(event.currentTarget as LoadingItem).id]!=null)
            {
				...
				// 如果加载的是 swf ，就不卸载了，其它的资源卸载掉
				var content:*;
				if((event.currentTarget as LoadingItem)._type == BulkLoader.TYPE_SWF)
				{
					content = loader.getContent( (event.currentTarget as LoadingItem).id);
				}
				else
				{
				...
				// bug 释放监听器
				(event.currentTarget as LoadingItem).removeEventListener(Event.COMPLETE,resourceLoaded);
				(event.currentTarget as LoadingItem).removeEventListener(BulkLoader.ERROR,resourceError);
				(event.currentTarget as LoadingItem).removeEventListener(BulkLoader.PROGRESS,resourceProgress);
				(event.currentTarget as LoadingItem).removeEventListener(BulkLoader.OPEN,resourceStarted);
            }
        }		
		...
		private function resourceProgress(event:Event):void
		{
			if (resources[(event.currentTarget as LoadingItem).id]!=null)
			{
				(resources[(event.currentTarget as LoadingItem).id] as Resource).progress(event);
			}
		}
		
		private function resourceStarted(event:Event):void
		{
			if (resources[(event.currentTarget as LoadingItem).id]!=null)
			{
				(resources[(event.currentTarget as LoadingItem).id] as Resource).started(event);
			}
		}
		...
		public function get getloader():BulkLoader
		{
			return loader;
		}
