...
package com.dnd
{
	...
	public class DragManager
	{
		...
		public static var s_context:Context;
		private static var _ignoreMouseUpOnce:Boolean; //忽略一次鼠标谈起事件。作用：一般都是在鼠标按下时，调用startDrag函数，对应于这次的鼠标
		 ...
		 */
		public static function startDrag(dragInitiator:Component, sourceData:SourceData, dragImage:DraggingImage = null, dragListener:DragListener = null, lockCenter:Boolean = false, ignoreMouseUpOnce:Boolean = true):void
		{
			if (s_isDragging)
			{
			...
			dragProxyMC.startDrag(lockCenter);
			...
			_ignoreMouseUpOnce = ignoreMouseUpOnce;
			
			s_context.m_gameCursor.setCmdState(GameCursor.CMDSTATE_DragItem);
		}
		...
		{
			UtilArray.removeFromArray(listeners, lis);
		}
		...
		{
			return getDropTarget(curStage, null, Component, ____dropTargetCheck) as Component;
		}
		...
		public static function getDropTarget(stage:Stage, pos:Point = null, targetType:Class = null, addtionCheck:Function = null):DisplayObject
		{
			...
			if (stage.areInaccessibleObjectsUnderPoint(pos) == true)
			{
				trace("affected");
			}
		...
		{
			if (_ignoreMouseUpOnce == true)
			{
				_ignoreMouseUpOnce = false;
				return;
			}
			var globalPos:IntPoint = new IntPoint(curStage.mouseX, curStage.mouseY);
			fireDragReadDropEvent(s_dragInitiator, s_sourceData, globalPos, enteredComponent);
		}
		...
		public static function drop():void
		{
			...
			s_sourceData = null;
			enteredComponent = null;
			s_dragInitiator = null;
			
			s_context.m_gameCursor.releaseCmdState(GameCursor.CMDSTATE_DragItem);
		}
		...
		private static function fireDragReadDropEvent(dragInitiator:Component, sourceData:SourceData, pos:IntPoint, targetComponent:Component):void
		{
			var e:DragAndDropEvent = new DragAndDropEvent(DragAndDropEvent.DRAG_READYDROP, dragInitiator, sourceData, pos, targetComponent);
			for (var i:int = 0; i < listeners.length; i++)
			{
				var lis:DragListener = listeners[i];
				lis.onReadyDrop(e);
		...
		public static function getDragInitiator():Component
		{
			return s_dragInitiator;
		}
