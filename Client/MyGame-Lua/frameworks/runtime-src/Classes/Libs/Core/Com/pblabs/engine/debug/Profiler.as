package com.pblabs.engine.debug
{
	...
	public class Profiler implements IProfiler
	{
		...
		public function Profiler(cnt:Context)
		{
			m_context = cnt;
		}
		...
		public function enter(blockName:String):void
		{
			...
			if (_stackDepth == 0)
			{
				...
				// the profiler.
				if (m_context.m_inputManager.isKeyDown(InputKey.P.keyCode))
				{
					if (!enabled)
					{
						_wantWipe = true;
				...
				// KBEN: 必然开启设置这里，测试 enter 和 exit 不匹配       
				//_reallyEnabled = true;
		...
		public function exit(blockName:String):void
		{
			...
			// bug: 退出的时候检查是否匹配，这个放在前面，否则没有开启 _reallyEnabled 的时候总是有问题，这个放在前面还有问题啊         
		...
		private function report_R(pi:ProfileInfo, indent:int):void
		{
			...
			// _rootNode.totalTime 是 0 ，那么 displayTime 值就是 NaN 
			if (pi.parent)
			...
			{
				entry = sprintf("%-" + (indent * indentAmount) + "s%-" + (nameFieldWidth - indent * indentAmount) + "s%-8s%-8s%-8s%-8s%-8s%-8s", "", (hasKids ? "+" : "-") + pi.name, pi.activations, displayTime.toFixed(2), displayNonSubTime.toFixed(2), (Number(pi.totalTime) / Number(pi.activations)).toFixed(1), pi.minTime, pi.maxTime);
			}
		 ...
		 */
		private var _reallyEnabled:Boolean = true;
		private var _wantReport:Boolean = false, _wantWipe:Boolean = false;
...
final class ProfileInfo
{
	...
	final public function wipe():void
	{
		...
		// 如果清除后，从没有进入这个块，那么 maxTime 值就是 int.MIN_VALUE，minTime 值就是  int.MAX_VALUE
