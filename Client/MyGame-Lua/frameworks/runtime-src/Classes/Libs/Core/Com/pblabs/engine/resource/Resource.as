package com.pblabs.engine.resource
{
	...
	// 进度事件
	[Event(name="PROGRESS_EVENT", type="com.pblabs.engine.resource.ResourceProgressEvent")]
	
	// 开始事件
	[Event(name="STARTED_EVENT", type="com.pblabs.engine.resource.ResourceEvent")]
    ...
    public class Resource extends EventDispatcher
    {
        ...
        public function initialize(data:*):void
        {
			...
			//var cnt:LoaderContext = new LoaderContext();
			//cnt.allowCodeImport = true;
            //loader.loadBytes(data, cnt);
			try
			{
			...
			{
				var strLog:String = "Resource::initialize(data:*) filename=" + this.filename + e.getStackTrace();
		...
		{
			var progressevent:ResourceProgressEvent = new ResourceProgressEvent(ResourceProgressEvent.PROGRESS_EVENT, this);
			progressevent.bytesLoaded = (event as ProgressEvent).bytesLoaded;
			progressevent.bytesTotal = (event as ProgressEvent).bytesTotal;
			progressevent._percentLoaded = (event as BulkProgressEvent)._percentLoaded;
		...
		{
			dispatchEvent(new ResourceEvent(ResourceEvent.STARTED_EVENT, this));
		}
		...
		// 用来手工触发加载成功事件，用在资源已经加载进来了，然后再次加载，并且至少有两个以上监听资源加载完成事件，这个时候需要手动触发这个事件
