	...
	public class polygonUtils
	{
		...
		public static function checkPolygonCollision(polygonA:Array, polygonB:Array):Boolean
		{
			...
			for (var edgeIndex:int = 0; edgeIndex < total; edgeIndex++)
			{
				...
				if (polygonUtils.IntervalDistance(boundA.min, boundA.max, boundB.min, boundB.max) > 0)
					result = false;
		...
		*/
		...
		public static function clipPolygon(polygon:Array, viewport:vport):Array
		{
			...
			var Clip_region:Array = [new Point(viewport.x_min, viewport.y_max), new Point(viewport.x_max, viewport.y_max), new Point(viewport.x_min, viewport.y_min), new Point(viewport.x_max, viewport.y_min)];
			...
			for (i = 1; i < nin; i++)
			{
				...
				else
				{
						...
						if (!((polygonUtils.M_code & ~0x100) & (polygonUtils.D_code & ~0x100)))
						{
							...
							if (polygonUtils.M_code & 0x100)
							{
								...
								while (j && maxLoops > 0)
								{
									...
									{
										if (A_code & polygonUtils.D_code)
											A_code = polygonUtils.M_code + polygonUtils.Tcc[A_code & ~0x100];
										else
											A_code = polygonUtils.D_code + polygonUtils.Tcc[A_code & ~0x100];
										j = 0;
									}
								}
							}
					...
					else
					{
						...
						if (polygonUtils.M_code & 0x100)
						{
							if (!(polygonUtils.M_code & polygonUtils.D_code))
								polygonUtils.D_code = polygonUtils.M_code + polygonUtils.Tcc[polygonUtils.D_code];
						}
						else
						{
							polygonUtils.D_code |= polygonUtils.M_code;
							if (polygonUtils.Tcc[polygonUtils.D_code] == 1)
								polygonUtils.D_code |= 0x100;
						}
					}
				}
		...
		private static function clipPolygonStep2(viewport:vport):int
		{
			...
			// Totally in ?
			if (polygonUtils.M_code == 0 && polygonUtils.D_code == 0)
				return 1;
		...
		private static function spaceCode(point_to_code:Point, viewport:vport):int
		{
			...
			{
				if (point_to_code.y < viewport.y_min)
					return (6 | 0x100);
				if (point_to_code.y > viewport.y_max)
					return (12 | 0x100);
				return (4);
			}
