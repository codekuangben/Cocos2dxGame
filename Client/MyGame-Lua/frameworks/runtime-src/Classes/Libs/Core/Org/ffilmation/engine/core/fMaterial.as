...
package org.ffilmation.engine.core
{
	...
	import org.ffilmation.engine.materials.fClipMaterial;
	import org.ffilmation.engine.materials.fDefaultMaterial;
	import org.ffilmation.engine.materials.fDoorMaterial;
	import org.ffilmation.engine.materials.fFenceMaterial;
	import org.ffilmation.engine.materials.fInvisibleMaterial;
	import org.ffilmation.engine.materials.fPerlinMaterial;
	import org.ffilmation.engine.materials.fTileMaterial;
	import org.ffilmation.engine.materials.fWindowMaterial;
	...
	public class fMaterial
	{
		...
		public var cls:fEngineMaterial; // The class that generates the material
		...
		public static function getMaterial(id:String, scene:fScene = null):fMaterial
		{
			...
			if (scene)
			{
				if (!fMaterial.usedMaterialsPerScene[scene.id])
					fMaterial.usedMaterialsPerScene[scene.id] = new Object;
			...
			else
			{
				...
				try
				{
					...
					{
						// KBEN: 现在需要场景 id
						//mat.definition = fMaterial.currentScene.engine.m_context.m_sceneResMgr.getMaterialDefinition(id);
						mat.definition = fMaterial.currentScene.engine.m_context.m_sceneResMgr.getMaterialDefinition(fMaterial.currentScene.m_serverSceneID, id);
						mat.type = mat.definition.type;
						mat.cls = null;
						if (mat.type == fMaterialTypes.TILE)
							mat.cls = new fTileMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.PERLIN)
							mat.cls = new fPerlinMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.DOOR)
							mat.cls = new fDoorMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.CLIP)
							mat.cls = new fClipMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.WINDOW)
							mat.cls = new fWindowMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.FENCE)
							mat.cls = new fFenceMaterial(mat.definition);
						else if (mat.type == fMaterialTypes.PROCEDURAL)
						{
							// NOTICE: 以后很可能添加到子域中，这个函数很可能没有效果了 
		...
		public static function disposeMaterials(scene:fScene):void
		{
			...
			if (mats)
			{
				for (var i:String in mats)
				{
					var mat:fMaterial = mats[i];
					mat.dispose();
					mats[i] = null;		// bug: 内存泄露
					delete mats[i];
				}
			}
			fMaterial.usedMaterialsPerScene[scene.id] = null;
			delete fMaterial.usedMaterialsPerScene[scene.id];
			if (fMaterial.currentScene == scene)
			{
				fMaterial.currentScene = null;
			}
		}
		...
		/** @private */
		//public function getDiffuse(element:fRenderableElement, width:Number, height:Number, fromPlane:Boolean = false):DisplayObject
		public function getDiffuse(element:fRenderableElement, width:Number, height:Number, fromPlane:Boolean = false):BitmapData
		{
			...
			// 直接返回 BitmapData 对象
			return this.cls.getDiffuse(element, width, height);
		}
