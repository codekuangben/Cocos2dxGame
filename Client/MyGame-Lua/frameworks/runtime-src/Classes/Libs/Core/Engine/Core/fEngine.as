package org.ffilmation.engine.core
{
	...
	public class fEngine extends EventDispatcher implements IResizeObject, IFrameStage
	{
		// Constants
		...
		private static var _shadowQuality:int = fShadowQuality.NORMAL;
		...
		public var m_context:Context;	// 这个是引擎里面的内容用来调用所有工具的过度的地方  
		 ...
		 */
		function fEngine(container:Sprite, context:Context):void
		{
			...
			m_context = context;
			m_context.m_sceneResMgr = new fSceneResourceManager(this);
			
			// KBEN: 添加到舞台大小改变处理队列中去，销毁的时候需要移除    
			m_context.m_processManager.addResizeObject(this, EntityCValue.ResizeScene);
			// 一帧结束的时候更新深度排序，没有卸载
			m_context.m_processManager.addFrameObject(this);
		}
		 ...
		 *
		 */
		public function loadMedia(src:String):void
		{
		...
		public function createScene(retriever:fEngineSceneRetriever, width:Number, height:Number, renderer:fEngineRenderEngine = null):fScene
		{
			...
			var nfScene:fScene = new fScene(this, nSprite, retriever, width, height, retriever.getServerSceneID(), renderer);
			nfScene.initialize();
		...
		*/
		...
		public function onResize(viewWidth:int, viewHeight:int):void
		{
			if (current)
			{
				current.setViewportSize(viewWidth, viewHeight);
			}
			
			var iscene:fScene;
			for each(iscene in scenes)
			{
				if (iscene && iscene != current)
				{
					iscene.setViewportSize(viewWidth, viewHeight);
				}
			}
		}
