...
package org.ffilmation.engine.core.sceneLogic
{
	...
	public class fCharacterSceneLogic
	{
		...
		public static function processNewCellCharacter(scene:fScene, character:fCharacter, forceReset:Boolean = false):void
		{
			...
			//character.updateFloorInfo(EntityCValue.TPlayer);
							...
							try
							{
								...
								for (i = 0; i < nEl && elementsV[i].distance < range; i++)
								{
									try
									{
										el = elementsV[i].obj;
										// Shadows of scene character upon other elements
										if (fCoverageSolver.calculateCoverage(el, character, x, y, z) == fCoverage.SHADOWED)
											cache.addElement(el);
									}
									catch (e:Error)
									{
			...
			*/
			...
			try
			{
				...
				for (var n:int = 0; n < nol; n++)
					if (newOccluding.indexOf(newOccluding2[n]) < 0)
						newOccluding[newOccluding.length] = newOccluding2[n];
			...
			for (i = 0; i < ool; i++)
			{
				...
				if (newOccluding.indexOf(oldOccluding[i]) < 0)
					scene.renderEngine.stopOcclusion(oldOccluding[i], character);
			}
			// 遮挡关闭
			if (character.occlusion >= 100)
			{
				if (scene.engine.m_context.m_profiler)
			...
			for (i = 0; i < nol; i++)
			{
				...
				if (oldOccluding.indexOf(newOccluding[i]) < 0)
					scene.renderEngine.startOcclusion(newOccluding[i], character);
			}
		...
		public static function renderCharacter(scene:fScene, character:fCharacter):void
		{
						...
						//{
							...
							//{
								...
								//{
									...
									//if (scene.prof)
										//scene.prof.end("S: " + light.id + " " + elements[i2].id);
								//}
								//else
								//{
									//scene.renderEngine.updateShadow(elements[i2], light, character);
			...
			{
				//if (scene.prof)
				//	scene.prof.begin("Occlusion");
				if (scene.engine.m_context.m_profiler)
					scene.engine.m_context.m_profiler.enter("Occlusion");
				var ocl:int = character.currentOccluding.length;
				for (i = 0; i < ocl; i++)
					scene.renderEngine.updateOcclusion(character.currentOccluding[i], character);
				//if (scene.prof)
				//	scene.prof.end("Occlusion");
				if (scene.engine.m_context.m_profiler)
					scene.engine.m_context.m_profiler.exit("Occlusion");
			}
			*/
			
			//if (scene.prof)
			//	scene.prof.end("Render char:" + character.id);
			if (scene.engine.m_context.m_profiler)
